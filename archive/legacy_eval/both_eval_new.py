...
def eval(dataset, dataloader, sampler, model, verbose=True, to_draw=True, draw_path= "", print_to_files=False, args = None):
    ...
    with torch.no_grad():
        ...
        if config.Detect_and_Estimate.type == "both_for_roots":
        ...
        for iter_num, data in enumerate(dataloader):
            ...
            if image_name=='T040_L053_2013.05.19_090054_004.jpg':
                a=1
            ...
            if len(gt_counts_temp)>0 or config.Detect_and_Estimate.type == "both_for_roots":
            ...
            if not config.detect_with_points.detect_points:
                ...
                if 'points_annot' in data.keys(): # true when evaluating during training
                    ...
                    detection_outputs, count_outputs, count_sample, relevant_points, crops_orig_boxes = \
                        model([image.cuda().float(), [data['bbox_annot'], None], torch.tensor(group_idx), True]) #image.cuda().float().unsqueeze(dim=0))
            ...
            if count_outputs is not None:
                ...
                per_im_pred_avg.append(current_sum/count_outputs[0].shape[0])
            ...
            if config.AttributeEstimation.estimate_type == 'withKeyPoints' and count_outputs is not None:
            ...
            if not isinstance(bbox_pred, list):
                    ...
                    for b in range(bbox_pred.shape[0]):
                       ...
                       #############################################################################################################
                        if config.Detect_and_Estimate.type == "both_for_roots":
                           detections_data_any_crop[image_name]['pred'].append(np.round(count_outputs[0][b][0].cpu().item())) # [0] - count, [1] - length, [2] - dia
                       ...
                       ############################################################################################################
                        ...
                        if config.Detect_and_Estimate.type == "both_for_roots":
                            count_pred.append(np.round(count_outputs[0][b][0].cpu().item()))
            ...
            if count_sample is not None or config.detect_with_points.detect_points:
                ...
                if max_gt_count_of_crop == -1:
                    count_sample = None
                else:
                    if config.AttributeEstimation.estimate_type == 'withKeyPoints' and not config.detect_with_points.detect_points:
            ...
            if count_sample is None and len(bbox_pred)>0:
                if verbose:
                    printf(
                        "##############################################################################################\n")
                    printf("image: %s\n", image_name)
                ...
                if len(box_annotations_withPoints)>0:
                    ...
                    for d in adjusted_crops_orig_boxes:
                        ...
                        if max_overlap >= config.Detection.iou_threshold and assigned_annotation not in detected_annotations:
                            ...
                            detections_data_any_crop[image_name]['gt_box_id'].append(-1)
                            ...
                            if len(crops_count_GT) > 0:
                                if crops_count_GT[i] != -1:
                                    ...
                                    if config.detect_with_points.detect_points:
                                        ...
                                        if len(orig_count_GT) > 0:
                                            if orig_count_GT[i] != -1:
                                                ...
                                                draw2.rectangle(((x1, y1), (x2, y2)), outline="blue",
                                                               width=config.DrawProperties.LINE_WIDTH)
                                    ...
                                    for j in range(len(relevant_points[i])): #points per box
                                        ...
                                        draw2.ellipse(((int(x)-5, int(y)-5), (int(x) + 5, int(y) + 5)), fill="black", width=config.DrawProperties.LINE_WIDTH)
        ...
        if total_orig_box_for_count>0:
            ...
            if config.Detect_and_Estimate.type == "both_for_roots":
                ...
                orig_avg_abs_dia_diff = SumOfAbsDifferences(total_orig_GT_dia, total_predicted_for_orig_boxes_dia) / total_orig_box_for_dia
        ...
        if verbose:
            ...
            if config.Detect_and_Estimate.type != "both_for_roots":
                ...
                print("Avg of per image rel_error:{:0.4f}".format(np.mean(abs_error)))
                ...
                for im in dia_per_im_pred_dict.keys():
                    ...
                    print("{}: sum_gt_dia: {:0.2f}, avg_pred_dia: {:0.2f}, rel_error_dia: {:0.2f}".format(im,
        ...
        if print_to_files and args is not None:
            if config.Detect_and_Estimate.type == "both_for_roots":
                csv_columns = ['img', 'pred', 'gt_count', 'label', 'score', 'pred_TRL', 'gt_TRL', 'pred_dia', 'gt_dia']
            else:
                csv_columns = ['img', 'pred', 'gt_count', 'label', 'score']
            csv_file = os.path.join(args.test_dir, "detections_data_any_crop.csv") #
            f = open(csv_file, 'w', newline='')
            with f:
                writer = csv.writer(f)
                writer.writerow(csv_columns)
