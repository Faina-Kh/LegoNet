...
def run(args=None):
    ...
    if args.run_script=='train':
                ...
                if args.dataset_type == "coco":
                    ...
                    if args.eval_in_train:
                        ...
                        if mAP > best_mAP:
                            best_mAP = mAP
                            util.printf("*\n")
                            torch.save(legonet, config.General.experiment_path+"legonet_epoch={}.pt".format(epoch_num))
                        else:
                            if epoch_num % 5 == 0:
                                torch.save(legonet, config.General.experiment_path + 'legonet_epoch={}.pt'.format(epoch_num))
                            util.printf("\n")
                    ...
                    if epoch_num % 10 == 0:
                        torch.save(legonet, os.path.join(config.General.experiment_path,"legonet_epoch={}.pt".format(epoch_num)))
                    ...
                    if args.evaluate_detection:
                        ...
                        if mAP > best_mAP:
                            ...
                            torch.save(legonet, os.path.join(config.General.experiment_path,
            ...
            #     count_agreement = counting_eval.eval(dataset_val, legonet, args)
                ...
                if args.eval_in_train:
                    ...
                    if rel_error < best_rel_error:
                        ...
                        torch.save(legonet, os.path.join(config.General.experiment_path, 'legonet_epoch={}.pt'.format(epoch_num)))  # checkpoints\\
                    # else:
                    #     if epoch_num % 5 == 0:
                    #         torch.save(legonet, os.path.join(config.General.experiment_path, 'legonet_epoch={}.pt'.format(epoch_num)))
                    ...
                    if (epoch_num+1) % config.General.SAVE_EVERY_N_EPOCHS == 0:
                        torch.save(legonet, os.path.join(config.General.experiment_path, 'legonet_epoch={}.pt'.format(epoch_num)))#cont_
        ...
        legonet.eval()
        torch.save(legonet, os.path.join(config.General.experiment_path, 'legonet_final.pt'))
