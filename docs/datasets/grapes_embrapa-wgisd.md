...
During dataset construction, LegoNet removes images without contributed berry
points and boxes that contain no contributed berry points. Detection evaluation
therefore covers only the retained berry-annotated images and their eligible
countable-cluster boxes; excluded images do not contribute false-positive detections.
...
In implementation terms, `filter_empty_bbox=True` performs this dataset
filtering, while `predict_empty_image=False` prevents per-object counting on an
empty annotated sample if one nevertheless reaches the model.
...
subject to software-version differences. The original MSR checkpoint was
unavailable; therefore, the released regression-based counting checkpoint was
produced by a newer training run and is not the checkpoint used for the paper
results.
...
D+R comparison uses the original keypoint checkpoint. The MSR comparison uses
a different, newly trained checkpoint, so its difference from the paper value
also reflects training variability.
...
- The grape architecture-ablation code and checkpoints are not included in the
  current release.
- Current-code results are subject to software-version differences and the
  availability of original checkpoints.

## License and citation

The annotation files in this directory are distributed under the
[Creative Commons Attribution-NonCommercial 4.0 International license
(CC BY-NC 4.0)](../../src/legonet/resources/datasets/Embrapa%20WGISD/LICENSE.txt),
separately from LegoNet's BSD-3-Clause source-code
