...
## Reporting and visual examples

Reference metrics should be stored as structured data and rendered as Markdown
tables rather than spreadsheet screenshots. Each table should identify whether
its rows are paper values or current-code outputs and explain incompatible
aggregation or split scopes.

Visual examples should be generated by the released code from a documented
checkpoint and public dataset. If a figure is copied from a paper, its caption
must identify the source and the repository must have permission to
redistribute it. Examples should be selected by a documented rule, such as
fixed image IDs, rather than chosen only because they look favorable.
