...
def eval(dataset, dataloader, sampler, model, verbose=True, to_draw=True):
    ...
    with torch.no_grad():
        ...
        for iter_num, data in enumerate(dataloader):
            ...
            if count_sample is not None:  # count_sample: corrected gt - has gt annotations for the crops  - for eval in training
                if 'points_annot' in count_sample.keys():
                    ...
                    if not isinstance(bbox_pred, list):
                        for b in range(bbox_pred.shape[0]):
                            ...
                            detections_data_any_crop[image_name]['pred'].append(count_pred[-1])
            ...
            if len(keep_not_found)>0:
                for i in keep_not_found:
                    not_found_gt[image_name].append(gt_counts_copy[i][0])
                ...
                for i in range(len(crops_count_GT)):
                    ...
                    if to_draw:
                        if not config.detect_and_count.crop_from_Pi:
                            ...
                            x1_crop = bbox_pred[i, 0]
                            y1_crop = bbox_pred[i, 1]
                            x2_crop = bbox_pred[i, 2]
                            y2_crop = bbox_pred[i, 3]
                            bbox_crop = roi_align(input=orig_img.unsqueeze(dim=0), boxes=[x1_crop,y1_crop,x2_crop,y2_crop], output_size=[80,80])
        ...
        if verbose:
            ...
            print("Avg of per image rel_error:{:0.2f}".format(np.mean(abs_error)))

        if config.General.MODE=='test':
            import csv
            #export detections info
            csv_columns = ['img', 'pred', 'gt_count', 'label', 'score']
            csv_file = os.path.join(config.General.experiment_path, "detections_data_any_crop.csv")
            ...
            csv_columns = ['img', 'not_found_gt_count']
