            ...
            if not config.detect_with_points.detect_points:
                ...
                if not args.have_GT:
                    detection_outputs, count_outputs, count_sample, relevant_points, crops_orig_boxes = \
                        model([image.cuda().float(), None, torch.tensor(group_idx), True])
                    input = [image.cuda().float(), None, torch.tensor(group_idx), True]

                elif 'points_annot' in data.keys():  # true when evaluating during training
                    detection_outputs, count_outputs, count_sample, relevant_points, crops_orig_boxes = \
                        model([image.cuda().float(), [data['bbox_annot'], data['points_annot']], torch.tensor(group_idx), True])
                    input = [image.cuda().float(), [data['bbox_annot'], data['points_annot']], torch.tensor(group_idx),
                             True]
                else:
                    detection_outputs, count_outputs, count_sample, relevant_points, crops_orig_boxes = \
                        model([image.cuda().float(), [data['bbox_annot'], None], torch.tensor(group_idx), True]) #image.cuda().float().unsqueeze(dim=0))
                    input = [image.cuda().float(), [data['bbox_annot'], None], torch.tensor(group_idx), True]
                    ...
                    for i in range(len(crops_count_GT)):
                        ...
                        if to_draw: # drawings of true detections
                            if len(crops_count_GT) > 0:
                                    ...
                                    if config.Counting.counting_type == 'withKeyPoints' and crops_count_GT[i] != 0 and draw_maps:
